...
async def test_criar_cliente_com_cpf_vazio(client):
    """Tentar criar um cliente com CPF vazio deve retornar erro 400 (Unprocessable Entity).

    Este teste verifica a validação de entrada na API: o campo CPF é
    obrigatório e não pode ser vazio. A resposta deve indicar que a
    requisição é inválida.
    """
    response = await client.post("/clientes", json={"cpf": "", "nome": "Cliente Y"})
