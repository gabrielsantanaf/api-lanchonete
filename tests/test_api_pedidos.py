...
async def test_fluxo_completo_pedido(client):
    ...
    # 4. Buscando o pedido pelo codigo e validando o retorno
    r1 = await client.get(f"/lanchonete/pedidos/{cod_pedido}")
    assert r1.status_code == 200
    assert r1.json()["cpf"] == "11122233344"

    # 5. Adiciona o segundo produto ao pedido já existente
    r2 = await client.put(f"/lanchonete/pedidos/{cod_pedido}/itens", json={"cod_produto": 2})
    assert r2.status_code == 200

    # 6. Finaliza o pedido e valida o total calculado
    r3 = await client.post(f"/lanchonete/pedidos/{cod_pedido}/finalizar")
    assert r3.status_code == 200
    assert r3.json()["total"] == 29.0

    # 7. Verificação de pedido com limite de produtos excedido
    #    Criamos um novo pedido com limite de 1 produto, tentamos adicionar outro
    r4 = await client.post("/lanchonete/pedidos", json={"cpf": "11122233344", "cod_produto": 1, "qtd_max_produtos": 1})
    assert r4.status_code == 200
    cod_pedido_limite = r4.json()["codigo"] # guarda o código do novo pedido
    r5 = await client.put(f"/lanchonete/pedidos/{cod_pedido_limite}/itens", json={"cod_produto": 2})
    assert r5.status_code == 400  # deve retornar erro de limite excedido
